...
int readMap(int mn)
{
  ...
  // if(readmapno == 0)fw.println("0");	// ⑦ファイルに書き込み
  // else if(readmapno == 1)fw.println("1");	// ⑦ファイルに書き込み
  // // fw.println("0");
  // // fw.print( String(mn) );	// ⑦ファイルに書き込み
  // //fw.print( "," );	// ⑦ファイルに書き込み
  // fw.close();	// ⑧ファイルを閉じる
  // delay(500);
...
// divnum回に分けて読み込む
  ...
  // // readStr = fr.readStringUntil(',');// ⑪改行まで１行読み出し
  // fr.close();	// ⑫	ファイルを閉じる
...
void dispTask(void *pvParameters) {
  ...
  if( ui.getEvent() != NO_EVENT ){//何かイベントがあれば
    ...
    if(readMapF == true){
      ...
      readMap(readmapno);//地図の読み込み
      // while(readMap(readmapno) == false){//地図の読み込み
      //   //マップの読み込みが完了するまで何もせずに待つ
      //   }
